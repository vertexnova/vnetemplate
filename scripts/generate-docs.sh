...
generate_api_docs() {
    ...
    mkdir -p "$BUILD_DIR"
    cd "$BUILD_DIR"
    cmake -DENABLE_DOXYGEN=ON "$PROJECT_ROOT"
...
case "${1:-}" in
    ...
    --api-only) check_prerequisites; generate_api_docs ;;
    --validate) check_prerequisites; validate_links; generate_coverage_report ;;
    "") main ;;
